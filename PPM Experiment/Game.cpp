...
Game::Game(Experiment::Trial trial) noexcept(false) : m_trial(std::move(trial))
{
	...
	m_controller = new Experiment::Controller(m_trial, m_deviceResources.get());
...
void Game::OnGamePadButton(DX::StepTimer const& timer)
{
	...
	auto next = m_controller->GetResponse(m_gamePad.get(), state);
	if (!next) return;
...
void Game::Update(DX::StepTimer const& timer)
{
	...
	m_controller->last_time = std::chrono::system_clock::now();
...
void Game::RenderStereo(const Experiment::DuoView& duo_view)
{
...
void Game::RenderStereo(const Experiment::SingleView& single_view)
{
	Render([=](int i, DirectX::SpriteBatch* spriteBatch)
	{
...
void Game::Render(const std::function<void(int, DirectX::SpriteBatch*)>& func)
{
	for (int i = 0; i < NUMBER_OF_WINDOWS; i++)
	{
